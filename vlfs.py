...
def validate_r2_connection(bucket: str = 'vlfs') -> bool:
    ...
    """
    # Ensure config is available (will raise ValueError if not)
...
def materialize_workspace(
    ...
    for rel_path, entry in entries.items():
        ...
        # Write atomically: create temp, then replace
...
def has_drive_token(vlfs_dir: Path) -> bool:
    ...
    """
    token_path = vlfs_dir / 'gdrive-token.json'
...
def write_rclone_drive_config(vlfs_dir: Path, config: dict[str, str]) -> None:
...
def auth_gdrive(vlfs_dir: Path) -> int:
    ...
    try:
        ...
        subprocess.run(
            ['rclone', 'config', '--config', str(config_file)],
            check=True
        )
        print()
    ...
    def do_upload_with_drive_retry():
        ...
        for attempt in range(5):
            try:
                ...
                if '403' in e.stderr or '429' in e.stderr or 'rateLimitExceeded' in e.stderr:
...
def main(argv: list[str] | None = None) -> int:
    ...
    auth_gdrive_parser = auth_subparsers.add_parser('gdrive', help='Authenticate with Google Drive')
    ...
    push_parser.add_argument('path', nargs='?', help='Path to file or directory to push')
    push_parser.add_argument('--private', action='store_true', help='Upload to private storage (Drive)')
    ...
    push_parser.add_argument('--all', action='store_true', help='Push all new or modified files')

    # status command
    status_parser = subparsers.add_parser('status', help='Show workspace status')
    status_parser.add_argument('--dry-run', action='store_true', help='Show what would be done without doing it')
    status_parser.add_argument('--json', action='store_true', help='Output in JSON format')
    status_parser.add_argument('--color', action='store_true', help='Force color output')

    # verify command
    verify_parser = subparsers.add_parser('verify', help='Verify workspace files against index')
    verify_parser.add_argument('--json', action='store_true', help='Output in JSON format')
    verify_parser.add_argument('--dry-run', action='store_true', help='Show what would be done without doing it')

    # clean command
    clean_parser = subparsers.add_parser('clean', help='Remove unreferenced cache objects')
    ...
    if args.command == 'status':
        ...
        if args.all:
            ...
            return cmd_push_glob(repo_root, vlfs_dir, cache_dir, args.glob, args.private, dry_run)
        elif args.path:
            return cmd_push(repo_root, vlfs_dir, cache_dir, args.path, args.private, dry_run)
        else:
...
def store_object(src_path: Path, cache_dir: Path, compression_level: int = 3) -> str:
    ...
    fd, temp_path = tempfile.mkstemp(dir=object_path.parent)
    try:
...
def write_index(vlfs_dir: Path, data: dict[str, Any]) -> None:
    ...
    fd, temp_path = tempfile.mkstemp(dir=index_path.parent, suffix='.json')
    try:
...
def cmd_status(repo_root: Path, vlfs_dir: Path, dry_run: bool = False, json_output: bool = False, force_color: bool = False) -> int:
    ...
    total_changes = len(status['missing']) + len(status['modified']) + len(status['extra'])
    
    if total_changes == 0:
        print(colorize("Workspace is up to date", 'GREEN', force_color))
    else:
        if status['missing']:
            print(f"{colorize('Missing:', 'RED', force_color)} {len(status['missing'])}")
        ...
        if status['modified']:
            print(f"{colorize('Modified:', 'YELLOW', force_color)} {len(status['modified'])}")
...
def _push_single_file_collect(
    ...
    if dry_run:
        ...
        try:
            if private:
                # Check Drive token
                if not has_drive_token(vlfs_dir):
            ...
            print(f"Uploaded: {rel_path} ({format_bytes(size)} -> {format_bytes(compressed_size)})")
        except RcloneError as e:
...
def cmd_verify(repo_root: Path, vlfs_dir: Path, dry_run: bool = False, json_output: bool = False) -> int:
    ...
    if json_output:
        ...
        if issues == 0:
            ...
            print(f"Verification: {colorize(f'{total - issues} OK', 'GREEN')}, " +
...
def cmd_clean(repo_root: Path, vlfs_dir: Path, cache_dir: Path, dry_run: bool = False, yes: bool = False) -> int:
    ...
    if dry_run:
        print(f"[DRY-RUN] Would delete {len(to_delete)} orphaned objects ({format_bytes(total_size)})")
...
def cmd_push_all(repo_root: Path, vlfs_dir: Path, cache_dir: Path, private: bool, dry_run: bool) -> int:
...
def cmd_push_glob(repo_root: Path, vlfs_dir: Path, cache_dir: Path, pattern: str, private: bool, dry_run: bool) -> int:
