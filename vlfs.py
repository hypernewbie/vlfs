...
def write_rclone_r2_config(dest_dir: Path) -> None:
    """Write rclone config for R2 using env vars.

    Args:
        dest_dir: Directory to write rclone.conf to (typically user config dir)
    """
    config_path = dest_dir / "rclone.conf"

    # Get credentials from environment
    r2_config = get_r2_config_from_env()

    # Build config lines for [r2] section
    config_lines = [
        "[r2]",
        "type = s3",
        "provider = Cloudflare",
    ...
    if "endpoint" in r2_config:
        config_lines.append(f"endpoint = {r2_config['endpoint']}")

    # Add credentials
    config_lines.append(f"access_key_id = {r2_config['access_key_id']}")
    config_lines.append(f"secret_access_key = {r2_config['secret_access_key']}")

    # Write atomically
    fd, temp_path = tempfile.mkstemp(dir=dest_dir, suffix=".conf")
    try:
...
def auth_gdrive(vlfs_dir: Path) -> int:
    ...
    try:
        ...
        # Write token file
        # Atomic write
        fd, temp_path = tempfile.mkstemp(dir=user_dir, suffix=".json")
        try:
            with os.fdopen(fd, "w") as f:
                f.write(token)  # Write raw token string (it's JSON content)
...
def cmd_pull(
    ...
    for remote, objects in remote_groups.items():
        ...
        missing = [
            key for key in object_keys if not (cache_dir / "objects" / key).exists()
        ]
        ...
        if missing:
            ...
            if remote == "r2" and r2_public_url:
                if dry_run:
                    ...
                    downloaded = download_from_r2_http(
                        missing, cache_dir, r2_public_url, dry_run
                    )
                    ...
                    try:
                        if remote == "gdrive":
                            ...
                            downloaded = download_from_drive(
                                missing, cache_dir, dry_run=False
                            )
                        else:
